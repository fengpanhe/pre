...
shared_ptr<ffm_model> train(
    ...
    vector<ffm_int> *cv_order_all = nullptr,
    ffm_int cv_vbegin = 0,
    ffm_int cv_vend = 0)
{
#if defined USEOMP
...
#pragma omp parallel for schedule(static) reduction(+: loss1)
#endif
                for(ffm_int ii = cv_vbegin; ii < cv_vend; ii++)
                {
                    ffm_int i = (*cv_order_all)[ii];
                ...
                if (param.ever_iter_avg) {
                    cout.width(13);
                    cout << fixed << setprecision(5) << loss/(cv_vend - cv_vbegin);
                }
            }
    ...
    ffm_parameter param)
{
    ...
    param.quiet = false;
    quiet = true;
    param.ever_iter_avg = true;     // 自定义变量打印每次交叉验证时的平均误差
    ...
    // 总数分为了nr_folds份, 每次取一份作为测试集, 其余作为训练集
    ...
    for(ffm_int fold = 0; fold < nr_folds; fold++)
    {
        ...
        // 将交叉验证中的"训练集分段"取出, 例如第0段为""与"50-200", 第1段为"0-50"与"100-200"
        //, 此数据将会作为训练数据
        ...
        shared_ptr<ffm_model> model
                    = train(prob, order1, param, nullptr, &order, begin, end);
